...
class TLErrorEvaluator(test: RequestPattern, testOn: Boolean, testOff: Boolean, deny: Boolean = false) extends Module
{
...
{
  def apply(test: RequestPattern, testOn: Boolean = false, testOff: Boolean = false, deny: Boolean = false): TLNode =
  {
    val errors = Module(new TLErrorEvaluator(test, testOn, testOff, deny))
    errors.node
  }
}
