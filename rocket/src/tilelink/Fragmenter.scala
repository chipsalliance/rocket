...
class TLFragmenter(val minSize: Int, val maxSize: Int, val alwaysMin: Boolean = false, val earlyAck: EarlyAck.T = EarlyAck.None, val holdFirstDeny: Boolean = false) extends Module
{
...
object TLFragmenter
{
  def apply(minSize: Int, maxSize: Int, alwaysMin: Boolean = false, earlyAck: EarlyAck.T = EarlyAck.None, holdFirstDeny: Boolean = false): TLNode =
  {
    if (minSize <= maxSize) {
      val fragmenter = Module(new TLFragmenter(minSize, maxSize, alwaysMin, earlyAck, holdFirstDeny))
      fragmenter.node
    } else { TLEphemeralNode()(ValName("no_fragmenter")) }
  }

  def apply(wrapper: TLBusWrapper): TLNode = apply(wrapper.beatBytes, wrapper.blockBytes)
}

// Synthesizable unit tests

class TLRAMFragmenter(ramBeatBytes: Int, maxSize: Int, txns: Int) extends Module {
  val fuzz = Module(new TLFuzzer(txns))
  val model = Module(new TLRAMModel("Fragmenter"))
  val ram  = Module(new TLRAM(AddressSet(0x0, 0x3ff), beatBytes = ramBeatBytes))
...
class TLRAMFragmenterTest(ramBeatBytes: Int, maxSize: Int, txns: Int = 5000, timeout: Int = 500000) extends UnitTest(timeout) {
  val dut = Module(Module(new TLRAMFragmenter(ramBeatBytes,maxSize,txns)).module)
