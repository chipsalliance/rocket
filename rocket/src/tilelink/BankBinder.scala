...
class BankBinder(mask: BigInt) extends Module
{
...
object BankBinder
{
  def apply(mask: BigInt): TLNode = {
    val binder = Module(new BankBinder(mask))
    binder.node
  }

  def apply(nBanks: Int, granularity: Int): TLNode = {
