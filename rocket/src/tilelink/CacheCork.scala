...
class TLCacheCork(params: TLCacheCorkParams = TLCacheCorkParams()) extends Module
{
...
object TLCacheCork
{
  def apply(params: TLCacheCorkParams): TLNode =
  {
    val cork = Module(new TLCacheCork(params))
    cork.node
  }
  def apply(unsafe: Boolean = false, sinkIds: Int = 8): TLNode =
  {
