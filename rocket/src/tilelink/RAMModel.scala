...
class TLRAMModel(log: String = "", ignoreCorruptData: Boolean = false, ignoreDeniedData: Boolean = true) extends Module
{
...
object TLRAMModel
{
  def apply(log: String = "", ignoreCorruptData: Boolean = false, ignoreDeniedData: Boolean = true): TLNode =
  {
    val model = Module(new TLRAMModel(log, ignoreCorruptData, ignoreDeniedData))
    model.node
  }
