...
class TLBroadcast(params: TLBroadcastParams) extends Module
{
  // Backwards compatibility
  def this(lineBytes: Int, numTrackers: Int = 4, bufferless: Boolean = false, filterFactory: TLBroadcast.ProbeFilterFactory = BroadcastFilter.factory) =
...
object TLBroadcast
{
  ...
  def apply(lineBytes: Int, numTrackers: Int = 4, bufferless: Boolean = false, filterFactory: TLBroadcast.ProbeFilterFactory = BroadcastFilter.factory): TLNode =
    apply(TLBroadcastParams(lineBytes, numTrackers, bufferless, filterFactory=filterFactory))
  def apply(params: TLBroadcastParams): TLNode =
  {
    val broadcast = Module(new TLBroadcast(params))
    broadcast.node
  }
}
