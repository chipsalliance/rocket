...
class TLRationalCrossingSource extends Module
{
...
class TLRationalCrossingSink(direction: RationalDirection = Symmetric) extends Module
{
...
object TLRationalCrossingSource
{
  def apply() =
  {
    val rsource = Module(new TLRationalCrossingSource)
    rsource.node
  }
}
...
{
  def apply(direction: RationalDirection = Symmetric) =
  {
    val rsink = Module(new TLRationalCrossingSink(direction))
    rsink.node
  }
}

@deprecated("TLRationalCrossing is fragile. Use TLRationalCrossingSource and TLRationalCrossingSink", "rocket-chip 1.2")
class TLRationalCrossing(direction: RationalDirection = Symmetric) extends Module
{
  val source = Module(new TLRationalCrossingSource)
  val sink = Module(new TLRationalCrossingSink(direction))
...
class TLRAMRationalCrossingSource(name: String, txns: Int) extends Module {
  val node = TLRationalIdentityNode()
  val fuzz  = Module(new TLFuzzer(txns))
  val model = Module(new TLRAMModel(name))
...
class TLRAMRationalCrossingSink(direction: RationalDirection) extends Module {
  val node = TLRationalIdentityNode()
  val ram  = Module(new TLRAM(AddressSet(0x0, 0x3ff)))
...
class TLRAMRationalCrossing(txns: Int) extends Module {
  val sym_fast_source = Module(new TLRAMRationalCrossingSource("RationalCrossing sym_fast", txns))
  val sym_slow_sink   = Module(new TLRAMRationalCrossingSink(Symmetric))
  sym_slow_sink.node := sym_fast_source.node

  val sym_slow_source = Module(new TLRAMRationalCrossingSource("RationalCrossing sym_slow", txns))
  val sym_fast_sink   = Module(new TLRAMRationalCrossingSink(Symmetric))
  sym_fast_sink.node := sym_slow_source.node

  val fix_fast_source = Module(new TLRAMRationalCrossingSource("RationalCrossing fast", txns))
  val fix_slow_sink   = Module(new TLRAMRationalCrossingSink(FastToSlow))
  fix_slow_sink.node := fix_fast_source.node

  val fix_slow_source = Module(new TLRAMRationalCrossingSource("RationalCrossing slow", txns))
  val fix_fast_sink   = Module(new TLRAMRationalCrossingSink(SlowToFast))
...
class TLRAMRationalCrossingTest(txns: Int = 5000, timeout: Int = 500000) extends UnitTest(timeout) {
  val dut = Module(Module(new TLRAMRationalCrossing(txns)).module)
