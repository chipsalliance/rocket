...
class TLPatternPusher(name: String, pattern: Seq[Pattern]) extends Module
{
...
{
  def apply(name: String, pattern: Seq[Pattern]): TLOutwardNode =
  {
    val pusher = Module(new TLPatternPusher(name, pattern))
    pusher.node
  }
}
