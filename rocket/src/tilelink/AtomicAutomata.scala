...
class TLAtomicAutomata(logical: Boolean = true, arithmetic: Boolean = true, concurrency: Int = 1, passthrough: Boolean = true) extends Module
{
...
{
  def apply(logical: Boolean = true, arithmetic: Boolean = true, concurrency: Int = 1, passthrough: Boolean = true): TLNode =
  {
    val atomics = Module(new TLAtomicAutomata(logical, arithmetic, concurrency, passthrough))
    atomics.node
  }
...
class TLRAMAtomicAutomata(txns: Int) extends Module {
  val fuzz = Module(new TLFuzzer(txns))
  val model = Module(new TLRAMModel("AtomicAutomata"))
...
class TLRAMAtomicAutomataTest(txns: Int = 5000, timeout: Int = 500000) extends UnitTest(timeout) {
  val dut = Module(Module(new TLRAMAtomicAutomata(txns)).module)
