...
{
  private val clockGroupAggregator = Module(new ClockGroupAggregator(busName){ override def shouldBeInlined = true }).suggestName(busName + "_clock_groups")
  private val clockGroup = Module(new ClockGroup(busName){ override def shouldBeInlined = true })
...
trait TLBusWrapperInstantiationLike {
  def instantiate(context: HasTileLinkLocations, loc: Location[TLBusWrapper]): TLBusWrapper
}

trait TLBusWrapperConnectionLike {
  val xType: ClockCrossingType
  def connect(context: HasTileLinkLocations, master: Location[TLBusWrapper], slave: Location[TLBusWrapper]): Unit
}
  ...
  extends TLBusWrapperConnectionLike
{
  def connect(context: HasTileLinkLocations, master: Location[TLBusWrapper], slave: Location[TLBusWrapper]): Unit = {
    ...
    def bindClocks = driveClockFromMaster match {
    ...
    def bindTLNodes = nodeBinding match {
...
{
  def instantiate(context: HasTileLinkLocations): Unit = {
    instantiations.foreach { case (loc, params) => context { params.instantiate(context, loc) } }
  }
  def connect(context: HasTileLinkLocations): Unit = {
...
trait HasTLXbarPhy { this: TLBusWrapper =>
  private val xbar = Module(new TLXbar).suggestName(busName + "_xbar")
  ...
  with TLBusWrapperInstantiationLike
{
  ...
  def instantiate(context: HasTileLinkLocations, loc: Location[TLBusWrapper]): AddressAdjusterWrapper = {
    val aaWrapper = Module(new AddressAdjusterWrapper(this, loc.name))
...
class AddressAdjusterWrapper(params: AddressAdjusterWrapperParams, name: String) extends TLBusWrapper(params, name) {
  private val address_adjuster = params.replication.map { r => Module(new AddressAdjuster(r, params.forceLocal, params.localBaseAddressDefault, params.ordered)) }
...
{
  val dtsFrequency = None
  def instantiate(context: HasTileLinkLocations, loc: Location[TLBusWrapper]): TLJBarWrapper = {
    val jbarWrapper = Module(new TLJBarWrapper(this, loc.name))
...
class TLJBarWrapper(params: TLJBarWrapperParams, name: String) extends TLBusWrapper(params, name) {
  private val jbar = Module(new TLJbar)
