...
class TLXbar_ACancel(policy: TLArbiter.Policy = TLArbiter.roundRobin) extends Module
{
...
object TLXbar
{
  ...
  def apply(policy: TLArbiter.Policy = TLArbiter.roundRobin): TLNode =
  {
    val xbar = Module(new TLXbar(policy))
    xbar.node
  }
...
object TLXbar_ACancel
{
  ...
  def apply(policy: TLArbiter.Policy = TLArbiter.roundRobin): TLNode_ACancel =
  {
    val xbar = Module(new TLXbar_ACancel(policy))
    xbar.node
  }
...
class TLRAMXbar(nManagers: Int, txns: Int) extends Module {
  val fuzz = Module(new TLFuzzer(txns))
  val model = Module(new TLRAMModel("Xbar"))
  val xbar = Module(new TLXbar)

  xbar.node := TLDelayer(0.1) := model.node := fuzz.node
  (0 until nManagers) foreach { n =>
    val ram  = Module(new TLRAM(AddressSet(0x0+0x400*n, 0x3ff)))
...
class TLRAMXbarTest(nManagers: Int, txns: Int = 5000, timeout: Int = 500000) extends UnitTest(timeout) {
  val dut = Module(Module(new TLRAMXbar(nManagers,txns)).module)
  dut.io.start := io.start
  io.finished := dut.io.finished
}

class TLMulticlientXbar(nManagers: Int, nClients: Int, txns: Int) extends Module {
  val xbar = Module(new TLXbar)
...
class TLMulticlientXbarTest(nManagers: Int, nClients: Int, txns: Int = 5000, timeout: Int = 500000) extends UnitTest(timeout) {
  val dut = Module(Module(new TLMulticlientXbar(nManagers, nClients, txns)).module)
  dut.io.start := io.start
  io.finished := dut.io.finished
}
