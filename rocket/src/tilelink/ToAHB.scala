...
import chisel3._
import chisel3.util.{Cat, Queue, RegEnable, log2Ceil}
...
class TLToAHB(val aFlow: Boolean = false, val supportHints: Boolean = true, val supportsRETRY: Boolean = true) extends Module
{
...
object TLToAHB
{
  ...
  {
    val tl2ahb = Module(new TLToAHB(aFlow, supportHints, supportsRETRY))
    tl2ahb.node
  }
}
