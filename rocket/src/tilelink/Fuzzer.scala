...
class TLFuzzer(
  ...
  nOrdered: Option[Int] = None) extends Module
{
...
object TLFuzzer
{
  def apply(
    ...
    noModify: Boolean = false,
    overrideAddress: Option[AddressSet] = None,
    nOrdered: Option[Int] = None): TLOutwardNode =
  {
    val fuzzer = Module(new TLFuzzer(nOperations, inFlight, noiseMaker, noModify, overrideAddress, nOrdered))
    fuzzer.node
  }
}
...
class TLFuzzRAM(txns: Int) extends Module
{
  val model = Module(new TLRAMModel("TLFuzzRAM"))
  val ram  = Module(new TLRAM(AddressSet(0x800, 0x7ff)))
  val ram2 = Module(new TLRAM(AddressSet(0, 0x3ff), beatBytes = 16))
  val gpio = Module(new TLRRTest1(0x400))
  val xbar = Module(new TLXbar)
  val xbar2= Module(new TLXbar)
...
class TLFuzzRAMTest(txns: Int = 5000, timeout: Int = 500000) extends UnitTest(timeout) {
  val dut = Module(Module(new TLFuzzRAM(txns)).module)
