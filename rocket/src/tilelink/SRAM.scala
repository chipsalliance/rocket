  ...
  ) extends DiplomaticSRAM(address, beatBytes, devName, dtsCompat)
{
  ...
  class Impl extends ModuleImp(this) with HasJustOneSeqMem {
...
object TLRAM
{
  ...
  {
    val ram = Module(new TLRAM(address, cacheable, executable, atomics, beatBytes, ecc, sramReg, devName))
    ram.node
  }
}
...
class TLRAMSimple(ramBeatBytes: Int, sramReg: Boolean, txns: Int) extends Module {
  val fuzz = Module(new TLFuzzer(txns))
  val model = Module(new TLRAMModel("SRAMSimple"))
  val ram  = Module(new TLRAM(AddressSet(0x0, 0x3ff), beatBytes = ramBeatBytes, sramReg = sramReg))
...
class TLRAMSimpleTest(ramBeatBytes: Int, sramReg: Boolean, txns: Int = 5000, timeout: Int = 500000) extends UnitTest(timeout) {
  val dut = Module(Module(new TLRAMSimple(ramBeatBytes, sramReg, txns)).module)
  dut.io.start := io.start
  io.finished := dut.io.finished
}

class TLRAMECC(ramBeatBytes: Int, eccBytes: Int, sramReg: Boolean, txns: Int) extends Module {
  val fuzz = Module(new TLFuzzer(txns))
  val model = Module(new TLRAMModel("SRAMSimple"))
...
class TLRAMECCTest(ramBeatBytes: Int, eccBytes: Int, sramReg: Boolean, txns: Int = 5000, timeout: Int = 500000) extends UnitTest(timeout) {
  val dut = Module(Module(new TLRAMECC(ramBeatBytes, eccBytes, sramReg, txns)).module)
  dut.io.start := io.start
  io.finished := dut.io.finished
}
