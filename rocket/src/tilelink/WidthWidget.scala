...
class TLWidthWidget(innerBeatBytes: Int) extends Module
{
...
{
  def apply(innerBeatBytes: Int): TLNode =
  {
    val widget = Module(new TLWidthWidget(innerBeatBytes))
    widget.node
  }
  def apply(wrapper: TLBusWrapper): TLNode = apply(wrapper.beatBytes)
}

// Synthesizable unit tests

class TLRAMWidthWidget(first: Int, second: Int, txns: Int) extends Module {
  val fuzz = Module(new TLFuzzer(txns))
  val model = Module(new TLRAMModel("WidthWidget"))
...
class TLRAMWidthWidgetTest(little: Int, big: Int, txns: Int = 5000, timeout: Int = 500000) extends UnitTest(timeout) {
  val dut = Module(Module(new TLRAMWidthWidget(little,big,txns)).module)
