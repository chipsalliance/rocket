...
class TLHintHandler(passthrough: Boolean = true) extends Module
{
...
{
  def apply(passthrough: Boolean = true): TLNode =
  {
    val hints = Module(new TLHintHandler(passthrough))
    hints.node
  }
}
...
class TLRAMHintHandler(txns: Int) extends Module {
  val fuzz = Module(new TLFuzzer(txns))
  val model = Module(new TLRAMModel("HintHandler"))
  val ram1  = Module(new TLRAM(AddressSet(0x0,   0x3ff)))
  val ram2  = Module(new TLRAM(AddressSet(0x400, 0x3ff)))
  val rom   = Module(new TLROM(0x800, 0x400, Seq.fill(128) { 0 }))
...
class TLRAMHintHandlerTest(txns: Int = 5000, timeout: Int = 500000) extends UnitTest(timeout) {
  val dut = Module(Module(new TLRAMHintHandler(txns)).module)
  io <> dut.io
}
