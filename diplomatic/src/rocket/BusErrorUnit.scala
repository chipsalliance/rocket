...
class BusErrorUnit[T <: BusErrors](t: => T, params: BusErrorUnitParams)(implicit p: Parameters) extends LazyModule {
  ...
  class Impl extends LazyModuleImp(this) {
    ...
    val (value, value_desc) = DescribedReg(sources.flatten.map(_.bits.getWidth).max.U,
      "value", "Physical address of error event", reset=None, volatile=true)
    require(value.getWidth <= regWidth)

    val enable = RegInit(init = VecInit(sources.map(_.nonEmpty.B)))
    ...
    val new_cause = WireInit(UInt(causeWidth.W), DontCare)
    val new_value = WireInit(UInt(value.getWidth.W), DontCare)
